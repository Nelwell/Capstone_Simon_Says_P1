...
def game_over(score):
    score -= 1
    # with open('simon_scores.txt') as f:
    #     print(score_list)
    if not path.exists('simon_scores.txt'):  # checks is file already exists
        with open('simon_scores.txt', 'a') as f:
            f.write('---Your High Scores---\n')  # adds header first time file created
            f.write(f'{score}\n')  # adds newest score to score sheet
            # score_list.append(f.read())
    else:
        with open('simon_scores.txt', 'a') as f:
            f.write(f'{score}\n')

    with open('simon_scores.txt') as f:
        next(f)  # skips header - https://stackoverflow.com/questions/4796764/read-file-from-line-2-or-skip-header-row
        score_list = f.readlines()  # puts scoring data from txt file into list
    print(score_list)
    no_newlines = ''.join(score_list).split()
    print(no_newlines)
    # score_list_numbers = int(no_newlines)
    # score_list_numbers.sort()
